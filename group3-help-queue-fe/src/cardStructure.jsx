...
const CardStructure = ({cardStuff , openTicketModal, deleteTicket, updateTicketToCompleted}) => {
    if (cardStuff.isCompleted === "true") {
    return (
        <div className="column" key={cardStuff.id}>
            <div className="card">
                <div className="ticket-user-time"><h3 className="inline">{cardStuff.usersName}</h3>      <p className="inline">{cardStuff.time}</p> <h3>{cardStuff.id}</h3><HiOutlineArrowsExpand style={{ verticalAlign: 'middle' }} size={28} className="expand" onClick={() => openTicketModal(cardStuff)}/></div>
